...
class SGL(GeneralGraphRecommender):
    ...
    # meta_contrast
    def meta_contrast(self,user_list, pos_item_list,user_subs,item_subs,meta_augs,mode):
        user_sub1,user_sub2=user_subs
        item_sub1,item_sub2=item_subs
        aug_1,aug_2=meta_augs
        # ----------------------step1--------------
        # ----------------------freeze the parameters of the augs 
        if mode=="step1":
            user_sub1_aug1=aug_1(user_sub1)
            user_sub2_aug1=aug_1(user_sub2)
            item_sub1_aug2=aug_2(item_sub1)
            item_sub2_aug2=aug_2(item_sub2)
            ori_ssl_loss=self.calc_ssl_loss(user_list, pos_item_list, user_sub1, user_sub2, item_sub1, item_sub2)
            aug_ssl_loss=self.calc_ssl_loss(user_list, pos_item_list, user_sub1_aug1, user_sub2_aug1, item_sub1_aug2, item_sub2_aug2)
            aug12_ssl_loss=self.calc_ssl_loss(user_list, pos_item_list, user_sub1, user_sub2_aug1, item_sub1, item_sub2_aug2)
            aug21_ssl_loss=self.calc_ssl_loss(user_list, pos_item_list, user_sub1_aug1, user_sub2, item_sub1_aug2, item_sub2)
            total_ssl_loss=ori_ssl_loss+aug_ssl_loss+aug12_ssl_loss+aug21_ssl_loss
        # ----------------------step2--------------
        ...
        # ----------------------freeze the parameters of the augs
            ...
            aug21_ssl_loss=self.calc_ssl_loss(user_list, pos_item_list, user_sub1_aug1, user_sub2, item_sub1_aug2, item_sub2)
            total_ssl_loss=ori_ssl_loss+aug_ssl_loss+aug12_ssl_loss+aug21_ssl_loss
        return total_ssl_loss

    def calculate_loss(self, interaction,meta_augs,mode):
        ...
        if mode=="step1":
            ...
            self.meta_contrast(user_list, pos_item_list,[user_sub1,user_sub2],[item_sub1,item_sub2],meta_augs,mode)
        # -------------------------------------step2---------------------------
        elif mode=="step2":
            user_emd, item_emd = self.forward()
            user_sub1, item_sub1 = self.forward(self.sub_graph1)
            user_sub2, item_sub2 = self.forward(self.sub_graph2)
            total_loss = self.meta_contrast(user_list, pos_item_list,[user_sub1,user_sub2],[item_sub1,item_sub2],meta_augs,mode)
        # --------------------------------------step3------------------------------
        else:
            user_emd, item_emd = self.forward()
            user_sub1, item_sub1 = self.forward(self.sub_graph1)
